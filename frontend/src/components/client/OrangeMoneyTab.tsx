...
export const OrangeMoneyTab: React.FC = () => {
  ...
  const isBlocked = orangeMoney?.status && orangeMoney.status !== 'ACTIF';
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 w-full">
        ...
        <div className="bg-white border-2 border-black p-6">
          ...
          <div className="mt-3 text-3xl font-black text-black font-mono">
            {(orangeMoney?.currentBalance ?? 0).toLocaleString()} <span className="text-sm font-bold">FCFA</span>
          </div>
        ...
        <div className="bg-white border-2 border-gray-200 p-6">
          ...
          <div className="mt-3 text-3xl font-black text-emerald-700 font-mono">
            {(orangeMoney?.savingsVaultBalance ?? 0).toLocaleString()} <span className="text-sm font-bold">FCFA</span>
          </div>
          <div className="mt-2 text-xs text-gray-500">
            Épargne sécurisée
          </div>
        ...
        <div className="bg-white border-2 border-gray-200 p-6">
          ...
          <div className="mt-3 space-y-1 text-xs">
            <div className="flex justify-between text-gray-600">
              <span>Jour :</span>
              <strong className="font-mono text-black">{(orangeMoney?.dailyLimit ?? 0).toLocaleString()} FCFA</strong>
            </div>
            <div className="flex justify-between text-gray-600">
              <span>Mois :</span>
              <strong className="font-mono text-black">{(orangeMoney?.monthlyLimit ?? 0).toLocaleString()} FCFA</strong>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border-2 border-gray-200 p-6 w-full">
        <div className="flex items-center justify-between pb-3 border-b border-gray-200 mb-4">
          ...
          <span className="text-xs text-gray-500">
            Total : {orangeMoney?.transactions?.length || 0} opération(s)
          </span>
        </div>

        {orangeMoney?.transactions && orangeMoney.transactions.length > 0 ? (
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-gray-900 bg-white px-2 py-0.5 border border-gray-200">
                      {tx.type || '-'}
                    </span>
                  </div>
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">
                    De : <strong>{tx.senderName || '-'}</strong> ({tx.senderMsisdn || '-'}) ➔ Vers : <strong>{tx.recipientName || '-'}</strong> ({tx.recipientMsisdn || '-'})
                  </div>
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <span className={`inline-block text-[10px] font-bold px-2 py-0.5 mt-1 ${
                    tx.status === 'SUCCESS' 
          ...
          <div className="text-center py-8 text-xs text-gray-400">
            Aucune transaction enregistrée pour ce compte Orange Money.
          </div>
        )}
